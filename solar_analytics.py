...
# Input features used by the machine learning models
FEATURE_COLUMNS = ["ghi", "temperature_c", "cloud_cover_pct"]
# Target variable to predict hourly solar energy production
...
# Convert solar irradiance into a normalized production factor
    irradiance_factor = ghi / 1000.0
# Reduce production based on cloud coverage
    cloud_derate = 1.0 - 0.0045 * cloud_cover
# Adjust efficiency depending on ambient temperature
    temperature_factor = 1.0 + cfg.temperature_coefficient * (
        temperature_c - cfg.nominal_cell_temperature_c
    )
# Estimate total photovoltaic power output in kW
    output_kw = (
        cfg.capacity_kwp
...
# Ensure production values cannot become negative
...
# Random Forest captures non-linear relationships in solar production
 ...
 # Linear regression pipeline with feature standardization
...
# Prevent negative production predictions
...
# Load a previously trained model if a model path is provided
    if model is None and model_path is not None:
        model = load_model(model_path)
# Use machine learning predictions when a trained model is available
 ...
 # Fall back to the physical simulation model otherwise
    else:
...
def calculate_economics(
    ...
    """
# Convert production and consumption data into pandas Series
...
# Revenue generated by selling excess electricity to the grid
...
def build_demo_model(
    ...
    """
# Load the existing dataset if available
    csv_path = Path(csv_path)
    if csv_path.exists():
        data = pd.read_csv(csv_path, parse_dates=["timestamp"], index_col="timestamp")
 # Otherwise, generate a synthetic solar dataset
    else:
...
# Run a complete training workflow when the script is executed directly
